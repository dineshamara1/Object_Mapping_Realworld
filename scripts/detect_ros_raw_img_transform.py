...
class Detector:
    ...
    def __init__(self):
        ...
        #Defining the topics
        self.image_topic = "/camera/rgb/image_color" #"/camera/rgb/image_color"
        self.depth_topic = "/camera/depth/image" #"/camera/depth/image"
        self.camera_frame = "openni_rgb_optical_frame"
        self.camera_topic = "/camera/depth/camera_info" #"/camera/depth/camera_info"
    ...
    def image_cb(self, data):
        ...
        if num_detections >= 1:
            ...
            for i in range(len(objects)):
                ...
                self.distance,self.object_wrt_world = self.uv_to_xyz(cx, cy)
                text1 = "distance:"
                text2 = str(round(self.distance,3))
                x = (int((dimensions[1])*image_height) + 60)
                y = int((dimensions[0])*image_width)
                cv2.putText(image, text1, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                cv2.putText(image, text2, (x,y+15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
                # Draw markers wrt world transform
                self.draw_markers(self.object_wrt_world)
